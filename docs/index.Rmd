...
<br><span style="font-size: 15px;">&nbsp;&nbsp;&nbsp;&nbsp;Note: Before I go into my analysis, here is a brief instruction on how to navigate the interactive visualization. Because the dataset is large, it will take a minute to load. Zoom in and out of the visualization and legend. Click on a single node to highlight its relationship with other nodes. Drag the cursor around to move around the network.Click on select id or degree centrality to filter through names or degree centrality.</span>
<br><span style="font-size: 18px;">&nbsp;&nbsp;&nbsp;&nbsp;I have produced three data visualizations for this dataset. The first one visualizes the relationship between all personalities in the database. The center is very dense, which may indicate the presence of individuals or small groups that act as connectors or hubs, playing a crucial role in linking various subgroups within the network. It also reveals information about strong and weak ties, namely blood-related and close-friendships are represented by dark colors and concentrated in certain areas. On the hand, friendships and acquaintances, shown with lighter edges, form a more extensive network of weaker ties. In addition, smaller clusters throughout the graph suggest the existence of distinct communities or subgroups, which may represent families, social circles, or groups connected by commonalities. Overall, the network appears to be somewhat balanced, with no single node or cluster overwhelmingly dominant. It is possible that it indicates a relatively decentralized structure where connections are evenly distributed across the network.
